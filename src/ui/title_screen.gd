...
func _ready() -> void:
	...
	# Full-rect column — never a CenterContainer card. The old 860×~720 card
	# sat flush with the 720p viewport and clipped QUIT / meta on short windows.
	...
	card.add_theme_stylebox_override("panel",
		UITheme.panel(UITheme.PANEL, UITheme.ACCENT, 1, 6, 16))
	...
	var title := UITheme.label("SALVAGE RUN", 42, UITheme.ACCENT, "Black")
	...
	if art != null:
		...
		bay.size_flags_stretch_ratio = 1.1
		bay.add_theme_stylebox_override("panel",
			UITheme.panel(UITheme.PANEL_DEEP, UITheme.ACCENT_DIM, 1, 4, 6))
		col.add_child(bay)
		...
		ship.custom_minimum_size = Vector2(0, 72)
		...
		gap.size_flags_stretch_ratio = 0.2
		col.add_child(gap)
	...
	stack.add_child(UITheme.label("STEAL A SHIP", 13, UITheme.TEXT_DIM, "Bold"))
...
func _ship_pick(choice: Dictionary) -> Control:
	...
	col.add_child(head)
	head.add_child(UITheme.label(String(choice["name"]).to_upper(), 15, UITheme.ACCENT, "Bold"))
	if not unlocked:
