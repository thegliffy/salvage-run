...
func _exit_tree() -> void:
	...
	if EventBus.intent_fizzled.is_connected(_on_fizzle):
		EventBus.intent_fizzled.disconnect(_on_fizzle)
...
func _build() -> void:
	...
	head.add_child(sp)
	_banner = UITheme.label("Fight %d of %d" % [Game.fight_index + 1, Game.LADDER.size()],
		14, UITheme.TEXT_DIM, "SemiBold")
...
func _build_enemy_panel() -> Control:
	...
	# Intent telegraph -- the most important readout on the screen.
	...
	col.add_child(UITheme.label("SUBSYSTEMS  —  click to target", 11, UITheme.TEXT_FAINT, "SemiBold"))
...
func _refresh() -> void:
	...
	var fizzling := combat.brain.intent_fizzles()
	_intent_panel.add_theme_stylebox_override("panel", UITheme.panel(
		Color("16241a") if fizzling else Color("2a1d16"),
		UITheme.GOOD if fizzling else UITheme.WARN, 1, 3, 9))
	_intent.add_theme_color_override("font_color",
		UITheme.GOOD if fizzling else UITheme.WARN)

	var intent_sys := StringName(combat.brain.current_intent.get("requires_system", ""))
	for sid in _enemy_views:
		_enemy_views[sid].refresh(e.systems[sid], sid == intent_sys and not fizzling)
...
func _on_fizzle(_who, intent: Dictionary) -> void:
	_log_line("✔ %s fizzles — the system that fires it is down!"
		% intent.get("telegraph", "Their attack"), UITheme.GOOD)
...
## Debug helper for the screenshot pass: play whatever is affordable at the
## enemy's firing subsystem, then end the turn. Not used by normal play.
func _debug_autoplay_turn() -> void:
	var target := StringName(combat.brain.current_intent.get("requires_system", ""))
	if target == &"" or not combat.enemy.has_active_system(target):
		var live := combat.enemy.targetable_systems()
		target = live[0].id if not live.is_empty() else &""
