...
func _ready() -> void:
	...
	wrap.add_theme_stylebox_override("panel",
		UITheme.panel(UITheme.PANEL, UITheme.ACCENT, 1, 6, 24))
	...
	var title := UITheme.label("DELIVERED TO THE YARD" if won else "WRECK TOWED IN",
		26, UITheme.GOOD if won else UITheme.WARN, "Black")
	col.add_child(title)
...
func _animate(v: Dictionary) -> void:
	...
	if not is_inside_tree():
		return
	_lines.add_child(UITheme.spacer(6))
	_lines.add_child(UITheme.hairline())
	_receipt_row("SUBTOTAL", "", str(v.get("subtotal", 0)), UITheme.TEXT_DIM, 14, "SemiBold",
		Color(), "Subtotal\n%d\n---\nParts plus bonuses, before the sector multiplier." % v.get("subtotal", 0))
	await get_tree().create_timer(0.14).timeout
	if not is_inside_tree():
		return
	_receipt_row(String(v.get("multiplier_label", "Sector")), "",
		"×%.2f" % v.get("multiplier", 1.0), UITheme.WARN, 15, "Bold", Color(),
	...
	total.add_theme_stylebox_override("panel",
		UITheme.panel_stripe(UITheme.INK_GOOD, UITheme.GOOD, 12))
	var trow := HBoxContainer.new()
	total.add_child(trow)
	var tl := UITheme.label("TOTAL", 20, UITheme.GOOD, "Black")
	tl.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	trow.add_child(tl)
	trow.add_child(UITheme.label("%d salvage" % v.get("total", 0), 22, UITheme.GOOD, "Black"))
